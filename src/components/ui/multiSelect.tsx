...
interface MultiSelectProps
  ...
  /**
   * If true, sets the default maximum width of the multi-select component to 24rem.
...
export const MultiSelect = React.forwardRef<HTMLButtonElement, MultiSelectProps>(
  (
    ...
    return (
      <div className={cn('relative grid w-full gap-2', defaultMaxWidth ? 'max-w-sm' : '')}>
        {labelName && (
                    ...
                    <div className="flex flex-wrap items-center">
                      {selectedValues.slice(0, maxCount).map((value) => {
                        ...
                        return (
                          <Badge
                            key={value}
                            className={cn(multiSelectVariants({ variant }), 'truncate')}
